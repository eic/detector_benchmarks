...
int dimuon_photoproduction_analysis(const string& filename, string outname_pdf, string outname_png, TString compact_file) {
    ...
    TH1D* hZ_proj = new TH1D("hZ_proj", "Muon track projections in nHCal; z [mm]; N_{proj}", NBINS, Z_MIN_MM, Z_MAX_MM);
    ...
    TH1D* hEsum = new TH1D("hEsum", "Reconstructed layer energy sum; E_{sum} [GeV]; N_{energy}", 10000, 0.0, 1.0);
    ...
    TCanvas* c_Eff = new TCanvas("c_Eff", "Muon efficiency analysis", 1600, 1000);
    c_Eff->Divide(2,2);                        
    c_Eff->cd(1); gPad->SetGrid(); hP_all_mu->SetLineColor(kBlack); hP_all_mu->Draw();  
    ...
    for (int idr=0; idr<3; ++idr) { 
        ...
        hEff_dr[idr]->Divide(hP_pass_dr[idr], hP_all_mu, 1, 1, "B");
    }   
    ...
    for (int ie=0; ie<3; ++ie) { 
        ...
        hEff_E[ie]->Divide(hP_pass_ECut[ie], hP_all_mu, 1, 1, "B");
    } 
    ...
    for (int il=0; il<3; ++il) { 
        ...
        hEff_Layer[il]->Divide(hP_pass_LayerCut[il], hP_all_mu, 1, 1, "B");
    } 
