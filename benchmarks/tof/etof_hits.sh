...
export JUGGLER_N_EVENTS=100
export JUGGLER_FILE_NAME_TAG="etof_hits"
...
if [ -z "${ANALYSIS_ONLY}" ] ; then
  ...
  root -b -q "benchmarks/tof/scripts/gen_etof_hits.cxx(${JUGGLER_N_EVENTS}, \"${JUGGLER_FILE_NAME_TAG}.hepmc\")"
  if [[ "$?" -ne "0" ]] ; then
...
if [ -z "${SIM_ONLY}" ] ; then
  ...
  root -b -q "benchmarks/tof/scripts/sim_etof_hits.cxx(\"${JUGGLER_SIM_FILE}\")"
  if [[ "$?" -ne "0" ]] ; then
