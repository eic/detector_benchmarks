...
int sim_tof_backward_hits(const char* fname = "sim_tof_backward_hits.root") {
  ...
  std::cout << "sim_tof_backward_hits: detector set to " << detector << std::endl;

  // minimal hit collection setup
    std::vector<std::pair<std::string, std::string>> hitCollections{{"tof_backward", "TOFEndcapHits"}};
  ...
  // Draw multiplicity versus theta
  {
    ...
    c.BuildLegend();
    c.SaveAs("results/tof/sim_tof_backward_hits_n_hits_vs_theta.png");
    c.SaveAs("results/tof/sim_tof_backward_hits_n_hits_vs_theta.pdf");
  }
  // Draw nhits
  {
    TCanvas c;
    ...
    hs.Draw("nostack, hist");
    c.BuildLegend();
    c.SaveAs("results/tof/sim_tof_backward_hits_n_hits.png");
    c.SaveAs("results/tof/sim_tof_backward_hits_n_hits.pdf");
  }
  ...
  // Draw total multiplicity versus theta
  {
    ...
    hth->DrawClone();
    c.BuildLegend();
    c.SaveAs("results/tof/sim_tof_backward_hits_theta.png");
    c.SaveAs("results/tof/sim_tof_backward_hits_theta.pdf");
  }

  for (auto& h : h2D) {
    TCanvas c;
    h->DrawClone("colz");
    c.SaveAs(fmt::format("results/tof/sim_tof_backward_hits_{}.png", h->GetName()).c_str());
    c.SaveAs(fmt::format("results/tof/sim_tof_backward_hits_{}.pdf", h->GetName()).c_str());
  }
