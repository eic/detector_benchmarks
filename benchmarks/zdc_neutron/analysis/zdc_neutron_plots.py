...
for p in momenta:
    arrays_sim[p] = ur.concatenate({
        f'sim_output/zdc_neutron/{config}_rec_zdc_neutron_{p}GeV_{index}.edm4eic.root': 'events'
...
dscalevals=[]
for p in momenta:
    selection=arrays_sim[p]["ReconstructedFarForwardZDCNeutrals.PDG"]==2112
...
plt.ylim(0.8, 1.2)
plt.ylabel("$\\mu[E_{n}]/E_{n}$")
plt.xlabel("$p_{n}$ [GeV]")
plt.axhline(1, ls='--', alpha=0.7, color='0.5')
plt.tight_layout()
plt.savefig(outdir+"neutron_energy_res.pdf")
...
for p in momenta:
    ...
    if p==100:
        ...
        plt.ylabel("events")
        plt.title(f"$p_{{n}}$={p} GeV")
        plt.xlabel("$\\theta^{n}_{recon}$ [mrad]")
    else:
